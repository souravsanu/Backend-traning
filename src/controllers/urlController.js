...
const shortenUrl = async function (req, res) {
    try {
        ...
        let urlInDB = await urlModel.findOne({ longUrl: originalUrl }).select({ _id: 0, updatedAt: 0, createdAt: 0, __v: 0 })
        if (urlInDB) return res.status(409).send({ status: false, message: "URL is already present", shortUrl: urlInDB.urlCode })
